...
def new_post_send(post_id):
    post = PostCategory.objects.get(pk=post_id)
...
def weekly_send_email_task():
    ...
    post = Post.objects.filter(dataCreation__gte=last_week)
    categories = set(post.values_list('postCategory__id', flat=True))
    subscribers = set(Subscriber.objects.filter(category__in=categories).values_list('user__email', flat=True))
    print(f'{subscribers = }')
